...
def main():
    ...
    # --- API Integration Tab ---
    tab1, tab2, tab3 = st.tabs(["🗺️ Main App", "🔐 API Integration", "📊 API Status"])
    ...
    with tab1:
        ...
        map_tab, gallery_tab = st.tabs(["🗺️ Interactive Map", "🖼️ Community Gallery"])
    ...
    with tab2:
        st.header("🔐 Indic Corpus Collections API Integration")
        st.markdown("""
        This section allows you to integrate with the Indic Corpus Collections API for:
        - **Professional Authentication** - Secure user management with OTP
        - **Data Synchronization** - Sync local submissions with the API
        - **Advanced Features** - Audio processing, content analysis, and more
        """)
        ...
        if not st.session_state.get("api_authenticated"):
            ...
            st.subheader("📝 API Records Management")
            ...
            with col1:
                if st.button("Get API Records"):
                    with st.spinner("Fetching records from API..."):
                        records = api_client.get_records(limit=10)
                        if "items" in records:
                            st.write(f"**Found {len(records['items'])} records:**")
                            for record in records['items']:
                                st.write(f"- {record.get('dialect_word', 'Unknown')} from {record.get('location_text', 'Unknown')}")
                        else:
                            st.error("Failed to fetch records")
            
            with col2:
                if st.button("Get API Categories"):
                    with st.spinner("Fetching categories..."):
                        categories = api_client.get_categories()
                        if "items" in categories:
                            st.write(f"**Found {len(categories['items'])} categories:**")
